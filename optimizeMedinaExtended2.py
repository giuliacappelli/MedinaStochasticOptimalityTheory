...
for typ in df_long.sentence.unique():		# content of "sentence" column in df_long
	...
	if "filler" in typ:
		...
		for iterat in df_long.iterativity.unique():	# content of "iterativity" column in df_long
			...
			tikzplotlib.save(outdir+'latex_boxplot_'+str(typ)+str(iterat)+'.tex')
			plt.close()
		...
		for mannsp in df_long.mannspec.unique():	# content of "mannspec" column in df_long
			...
			tikzplotlib.save(outdir+'latex_boxplot_'+str(typ)+str(mannsp)+'.tex')
			plt.close()
...
df_judg.to_csv(args.output+"df_judg_norm_wide"+".csv")
...
for sps in os.listdir(args.sps):
	...
	# Zipf*PISA values, in order to account for frequency effects (-.85 Zipf~PISA Pearson correlation)
	...
	df_sps_zscores = df_sps[numeric_cols].apply(zscore) # computes z-scores
	...
	df.to_csv(outfolder+"df_zmeans_"+os.path.basename(sps)+".csv")
	...
	df_aggr.to_csv(outfolder+"df_scatterplot_"+os.path.basename(sps)+".csv")
...
# preliminary stats and plots to see individual and overall contribution of predictors to the actual judgments
	...
	plt.savefig(outdir+'plot_star_boxplot_telicity.png')
	tikzplotlib.save(outdir+'latex_star_boxplot_telicity.tex')
	plt.close()
	...
	add_stat_annotation(ax, data=df, x="perfectivity", y="judg",
				...
				test='Mann-Whitney', text_format='star', loc='outside', verbose=0) # text_format='star', verbose=2, loc='inside'
	plt.savefig(outdir+'plot_star_boxplot_perfectivity.png')
	tikzplotlib.save(outdir+'latex_star_boxplot_perfectivity.tex')
	plt.close()
	...
	add_stat_annotation(ax, data=df, x="iterativity", y="judg",
				...
				test='Mann-Whitney', text_format='star', loc='outside', verbose=0) # text_format='star', verbose=2, loc='inside'
	plt.savefig(outdir+'plot_star_boxplot_iterativity.png')
	tikzplotlib.save(outdir+'latex_star_boxplot_iterativity.tex')
	plt.close()
	...
	add_stat_annotation(ax, data=df, x="mannspec", y="judg",
				...
				test='Mann-Whitney', text_format='star', loc='outside', verbose=0) # text_format='star', verbose=2, loc='inside'
	plt.savefig(outdir+'plot_star_boxplot_mannspec.png')
	tikzplotlib.save(outdir+'latex_star_boxplot_mannspec.tex')
	plt.close()
...
# ----------------------------------------------------------------------------------
	...
	df_Iter = df[df.values == 'iter']
	df_Noiter = df[df.values == 'noiter']
	df_Spec = df[df.values == 'spec']
	df_Nospec = df[df.values == 'nospec']
	...
	with open(outfolder+'errors.txt', 'w') as file_errors:
		...
		k = 5 # number of variables in the model (independent regressors): sps, tel, perf, mannspec, iter
	...
	tikzplotlib.save(outfolder+'latex_prob_noniterarg.tex')
	plt.close()
	...
	tikzplotlib.save(outfolder+'latex_prob_mannspecarg.tex')
	plt.close()
	...
	plt.plot([sps_min, sps_max], [constantsMedina[7], constantsMedina[6]], 'k-', label='p (*INT ARG >> NON-ITER ARG)')
	plt.plot([sps_min, sps_max], [constantsMedina[9], constantsMedina[8]], 'r--', label='p (*INT ARG >> MANN SPEC ARG)')
	plt.xlabel(sps_name)
	plt.xticks([sps_min, sps_max])
	plt.yticks(np.arange(0, 1.1, 0.1))
	plt.grid(color='grey', linestyle='dotted', linewidth=0.2, axis='y')
